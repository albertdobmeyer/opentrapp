...
- **DNS rebinding against allowlisted hosts is now a narrow, kernel-backstopped residual.** The proxy's allowlist matcher operates on the request's `Host` header (a domain string) and rejects raw IP-literal hosts in the goproxy matcher (`IsAllowed` in [`policy/policy.go`](../infra/proxy/goproxy/policy/policy.go), regression-pinned by [`policy/policy_test.go`](../infra/proxy/goproxy/policy/policy_test.go)). On top of that hostname check, the structural fix has **shipped**: a post-resolve destination-IP filter (`IsPrivateIP`, gated in `DecideRequest`) now runs *after* the allowlist check, so an allowlisted domain whose authoritative DNS server returns a private/loopback address (e.g. `127.0.0.1`, `172.17.0.1`, an RFC1918 range, or AWS metadata `169.254.169.254`) is rejected before the proxy connects; it is unit-tested in [`policy/policy_test.go`](../infra/proxy/goproxy/policy/policy_test.go). This is layered with a kernel-level RFC1918 egress filter in the dedicated `vault-egress` sidecar ([ADR-0009](adr/0009-five-container-perimeter.md) Tier 4) and a pinned DoT resolver (`unbound` forwarding only to Quad9 and Cloudflare over DNS-over-TLS; [ADR-0010](adr/0010-pinned-resolver-dns.md)). The earlier discussion of mitmproxy's `block_private` flag no longer governs this row: those flags are **source-IP filters** that gate which client IPs may use the proxy, not destination filters, and `block_private=false` remains set in [`compose.yml`](../compose.yml) only because the agent container's source IP is itself private (it lives on an internal podman network); destination filtering is now done explicitly by `IsPrivateIP` plus the `vault-egress` kernel drop rather than relying on mitmproxy. **Honest residual:** a DNS-rebinding TOCTOU between the proxy's resolve and its connect remains theoretically possible (the resolved IP could change in the window between the check and the socket connect), but it is now backstopped at the kernel layer by the nftables RFC1918 drop in `vault-egress`. Full live re-verification of the layered behavior on capable hardware is pending (this workstation swap-storms under the full perimeter), so the end-to-end claim is treated as shipped-but-not-yet-live-re-verified.
...
| Read the API credential from the proxy's request log | I | The substitution is byte-replace; the literal credential is never logged | None: verified by inspection of the goproxy (`RedactKeys`/`RedactURL` in `policy.go`) | `grep -E '(API_KEY|x-api-key)' vault-proxy/var/log/vault-proxy/requests.jsonl` returns nothing |  Same |
...
|------|--------|-----------|
| Post-resolve destination-IP filtering inside the goproxy (`IsPrivateIP`, T3 DNS-rebinding row) | Shipped; unit-tested in [`policy/policy_test.go`](../infra/proxy/goproxy/policy/policy_test.go) | [`adr/0009-five-container-perimeter.md`](adr/0009-five-container-perimeter.md) Tier 2 |
