...
- **Your API key being stolen by the agent** (T1). The expensive vendor
  ...
  container**, so a compromised agent has nothing to steal.
...
- **The agent's own reasoning being fooled.** Prompt-injection defense *inside* the
  ...
  prevent it.
- **A stolen control channel.** Whoever controls the account paired to the agent
  controls the agent. (The OpenClaw recipe adds a Telegram gateway, so for it the
  channel is a Telegram account.) **Use a dedicated account with two-factor**, not
  your personal one.
- **A few known, documented gaps**, tracked openly in the threat model:
  ...
  - **DNS rebinding** against an allowlisted domain is a *partial* residual risk;
    the kernel egress filter and DoT resolver narrow it but it is not fully
...
2. **Pair a dedicated account** to the agent's control channel with two-factor,
   never your personal one. (For the OpenClaw recipe, that channel is a Telegram
   account.)
