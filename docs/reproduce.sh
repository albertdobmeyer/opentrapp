...
patterns=$(grep -cE "^\s*'(CRITICAL|HIGH|MEDIUM)\|" components/clawhub-forge/tools/lib/patterns.sh 2>/dev/null || echo "0")
...
verify_checks=$(
  {
    grep -oE '^check [0-9]+' components/openclaw-vault/scripts/verify.sh \
      | grep -oE '[0-9]+'
    grep -oE 'printf "  \[%2d\][^"]*" [0-9]+' components/openclaw-vault/scripts/verify.sh \
      | grep -oE '[0-9]+$'
