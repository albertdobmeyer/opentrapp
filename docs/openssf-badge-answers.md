...
Important: do not press the form's "does not use cryptographic mechanisms" button. That button would set every cryptography criterion to not applicable, which would be inaccurate. The software does use cryptographic mechanisms: the desktop updater verifies a minisign signature before applying an update (the pubkey is in tauri.conf.json), the bootstrap verifies container images against a signed digest overlay at first launch, releases are signed with Sigstore cosign, and transport uses TLS. The honest answer is to fill the cryptography criteria individually as below: Met for the six that describe correct use of standard cryptography through established libraries, and not applicable only for crypto_pfs (no project-specific key-agreement protocol), crypto_password_storage (no user accounts), and crypto_random (the software verifies signatures but does not itself generate keys or nonces).
