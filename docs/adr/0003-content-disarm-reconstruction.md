...
**(c) The pipeline is auditable; the rebuild is best-effort, not bit-identical.** The static scan, line classification, and quarantine outcomes are deterministic. The intent-extraction and reconstruction step is model-backed, so re-running CDR on the same source can produce a differently-worded rebuild — and a describe-with-repair loop deliberately re-runs the model when validation fails. What is guaranteed is the *property*, not the bytes: the original is never delivered, every delivery is post-verified (re-scanned, re-classified) and SHA-256 signed, and any failure quarantines. Auditability comes from the signed clearance report — which input produced which signed, verified output — not from byte-for-byte reproducibility. (Earlier drafts of this ADR claimed per-input determinism; the shipping pipeline's repair loop makes that false, so the claim is corrected here.)

**(d) The deterministic stages run offline; only the model-backed rebuild needs an endpoint.** Scan, line-classification, parse, and pre-filter make no network access — the pattern catalogue, the safe-line classifier, and the parser are all in-image. The external interactions are (1) the initial download into quarantine and (2) the model-backed intent-extraction/reconstruction step (see (c)), which calls a **configurable** endpoint (`config/cdr.conf` — a local Ollama model or any OpenAI-compatible API; bring-your-own, no cloud lock-in). Consequently the standalone Skill Firewall's **Tier A** (`skill scan` / `skill verify`) is fully offline; only **Tier B** (the CDR rebuild) needs a model. (Earlier drafts claimed the *whole* pipeline ran offline, including "rebuild"; that contradicts (c) and is corrected here.)
...
The implementation lives in `workloads/skills/tools/skill-cdr.sh` (the pipeline driver) and the `tools/lib/cdr-*` scripts (the per-stage primitives — pre-filter, intent extraction, parser, reconstructor, validator).
