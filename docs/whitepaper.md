...
The first four containers each run on their own Docker `internal: true` network. `vault-proxy` bridges those internal networks and reaches outbound only via `egress-net` to `vault-egress`. `vault-egress` is the only container on `external-net`. `vault-agent` cannot reach `vault-forge` or `vault-pioneer` through any routed path; the only means by which a certified skill reaches `vault-agent` is the write-only shared volume `forge-deliveries`. The L7/L3 split is the load-bearing security property: no single container holds both API credentials *and* elevated network capabilities.
