...
**A practical perimeter architecture for running autonomous CLI agents on a personal computer without unrestricted host access. The reference integration ships pre-wired for [OpenClaw](https://www.getopenclaw.ai); opencode and Claude Code are candidates for support, and the architecture is designed to extend to other CLI agents.**

Albert Dobmeyer, independent
albertdobmeyer@proton.me

v0.9.0 (current development line). Created 2026-05-04. Companion repository: [github.com/albertdobmeyer/opentrapp](https://github.com/albertdobmeyer/opentrapp)

> **Currency note (2026-06-27).** This paper was written in the v0.7.2 development line. Since then the **de-Tauri cutover shipped on `main`** ([#184](https://github.com/albertdobmeyer/opentrapp/pull/184), 2026-06-24): OpenTrApp is **no longer a desktop application.** The product is the headless **`opentrapp-daemon`** that owns the perimeter, with an **optional on-demand browser viewer** (the loopback `viewer-server`) as the GUI projection ([ADR-0019](adr/0019-headless-daemon-gui-viewer-split.md) / [ADR-0022](adr/0022-daemon-control-surface.md)); the credential-holding `vault-proxy` is now the lean Go **`goproxy`** chokepoint, replacing the Python mitmproxy ([ADR-0026](adr/0026-vault-proxy-replacement-evaluation.md)). **The security architecture this paper describes — the five-container perimeter, the L7/L3 split, privilege separation, CDR — is unchanged.** Where the text below says "desktop application" / "Tauri" (notably the §7 implementation detail), read it as the *pre-cutover delivery*; the current architecture is canonical in [`trifecta.md`](trifecta.md). The CLI-first / registry / optional-MCP command surfaces remain forthcoming ([ADR-0020](adr/0020-product-identity-and-distribution.md)).

---

## Abstract

Autonomous CLI agents that can execute shell commands, read user files, browse the web, and load third-party skills present a containment problem of practical concern: their default operating mode grants them the same operating-system privileges as the human user, so any compromise (through prompt injection, a malicious skill, or a flaw in the agent itself) translates directly into damage to the user's host. Existing hardening guidance for these runtimes treats containment as a configuration problem (set `tools.deny`, set `sandbox.mode`, restrict the API key); empirical evidence suggests this layer alone is insufficient. The ClawHavoc study of 2026-Q1 classified 11.9 % of published ClawHub skills as malicious; CVE-2026-25253 demonstrated a one-click remote-code-execution vector through OpenClaw's management API; the Moltbook database breach exposed 1.5 M API tokens via a single misconfiguration.

This paper describes OpenTrApp. It runs an autonomous CLI agent inside a five-container security perimeter on the user's own computer — delivered as a headless `opentrapp-daemon` with an optional on-demand browser viewer (de-Tauri; see the currency note above). The perimeter is built on two ideas: privilege separation (no single container holds both the API keys and internet access) and supply-chain defense (every skill is vetted in isolation before the agent loads it). The reference deployment uses OpenClaw; opencode and Claude Code are candidates for support, and the architecture is designed to extend to other CLI agents. It provides defense-in-depth across three independent threat categories (runtime compromise, supply-chain attack, hostile network or feed content), exposes a stateful adaptive shell that allows the agent's privilege level to be modulated per task context, and ensures that the user's API credentials are held by a dedicated proxy container and never reach the agent's runtime. Two design choices in particular distinguish this work from prior hardening guidance: *proxy-side credential injection*, which keeps the literal API-key value invisible to the agent regardless of compromise, and *Content Disarm and Reconstruction* applied to skills, which discards the original artefact and rebuilds a clean version from the parsed semantic intent.

The implementation runs in two-container (vault-agent + vault-proxy) and five-container (adding vault-skills for skill scanning, vault-social for feed-content analysis, and vault-egress for kernel-level egress policy plus pinned DoT DNS) configurations. The L7/L3 policy split between vault-proxy and vault-egress is the load-bearing security property of the five-container topology: no single container holds both API credentials *and* elevated network capabilities. See [ADR-0009](adr/0009-five-container-perimeter.md) for the rationale. In the current development line the perimeter passes a 24-point startup verification, a 115-check manifest-orchestration suite, and an end-to-end browser harness. The daemon owns the perimeter's lifecycle: every termination path validated in dogfooding (graceful exit, SIGTERM, SIGINT, SIGKILL, OS reboot) tears down the perimeter cleanly or recovers it on next launch.
...
Empirical evidence from the agent's first quarter of release makes the cost of this attack surface concrete:
...
This paper describes a different approach: a five-container *perimeter* that runs alongside the agent runtime, owns the user's credentials in a separate sidecar process, mediates every outbound request at both the application layer (L7: domain allowlist, key injection, post-resolve IP check) and the network layer (L3: kernel-level RFC1918 drop plus pinned DoT resolver), scans every loaded skill before the agent sees it, and provides defense-in-depth that survives a single-layer compromise. The perimeter rests on two ideas: privilege separation (no single container holds both the API keys and internet access) and supply-chain defense (every skill is vetted in isolation before the agent loads it). Its lifecycle is owned by the headless `opentrapp-daemon`: start the daemon (`opentrapp-daemon vault up`), the perimeter comes up; stop the daemon, the perimeter cleanly tears down; OS-level kill, the perimeter is reaped on next launch. The optional on-demand browser viewer is a projection, not the lifecycle owner.

The contribution of this work is not a single novel mechanism. The individual primitives (Linux capability dropping, seccomp filtering, network isolation, allowlist proxies, content sanitisation) are each well established. The contribution is the *composition*: these primitives integrated into a coherent perimeter that an end user can install with a setup wizard, control from a Telegram bot, and reason about with a single clearly-bounded surface (`compose.yml` plus the workload manifests). Two specific composition choices are non-obvious in a way that warrants detailed treatment in §5 and §6: *proxy-side credential injection* and *Content Disarm and Reconstruction applied to skills*.
...
The perimeter is designed to address seven attacker categories, summarised below. This summary tracks the canonical model in [`docs/threat-model.md`](threat-model.md), which is the single source of truth and decomposes each category by STRIDE class with explicit residual-risk annotations and per-row evidence citations.

**T1: Prompt-injection author.** An attacker who controls content the agent will eventually see and process: a Telegram message body, an HTTP response from a fetched URL, a piece of feed content, the body of a loaded skill. Capability: cause the agent to execute any tool the agent currently has enabled, exfiltrate data, or modify state. Mitigation goal: limit the *set* of enabled tools (adaptive shell, §5) and the *destinations* tools can reach (proxy allowlist, §4.1). Hostile feed content is one delivery vector for prompt injection and belongs here; the optional `vault-social` shield (§4.3) is a second instance of the content-vetting pattern applied to that vector.

**T2: Malicious skill author.** An attacker who publishes a skill on ClawHub designed to look useful and carry a payload. Capability: gain code execution inside the agent's runtime when the skill is loaded. Mitigation goal: prevent malicious skills from reaching the agent's runtime in usable form (vault-skills scanner plus CDR pipeline, §6) and limit the blast radius of any skill the scanner missed (container hardening, §4.2).

**T3: Network man-in-the-middle (MITM).** An attacker positioned between vault-proxy and the public internet (e.g. a compromised local network or a hostile DNS). Capability: read or modify the agent's outbound traffic. Mitigation goal: TLS termination of all outbound calls inside vault-proxy with certificate pinning where supported; egress restricted to a small allowlist so a successful MITM is detectable.

**T4: Compromised host.** An attacker who already has partial access to the user's machine: a malicious binary the user installed before this perimeter, an OS-level rootkit, or another foothold. Capability depends on the attack vector but is generally significant. Mitigation goal: explicitly out of scope. The perimeter assumes the host is honest; if the host is dishonest, no container-level isolation can recover security. Users requiring stronger isolation are directed to run the perimeter on a disposable virtual machine (see the README's *Limitations* section and the per-module isolation-tiers documentation).
...
**T6: Side-channel observer.** An entity with read access to artefacts the perimeter produces outside the five containers: proxy request logs, system metrics, container layer caches, image metadata, and the application's own state files. Capability: infer *what the user is doing with the agent* from conversational metadata, request timing, and activity patterns even when the credential and workspace are structurally protected. Mitigation goal: keep the literal credential out of every artefact (proxy-side injection guarantees it never appears in logs); document what persists and how to erase it (§9).

**T7: Prompt-injected host operator (Proposed).** A trusted, user-installed host agent (e.g. Claude Code or opencode) that is itself prompt-injected and that can reach an OpenTrApp control surface, aiming to weaken the perimeter that protects the *contained* agent. This is the T1 attack aimed one level up, at the external operator. It is defined in [ADR-0021](adr/0021-danger-gated-agentic-control-plane.md) and folded into the canonical model on that ADR's acceptance. The in-scope guarantee is narrow and honest: OpenTrApp's agentic control plane is never an *amplifier*. Boundary-weakening through any surface requires the same out-of-band human confirmation whether the caller is a human or an agent. A fully privileged, fully injected host agent that tampers with `~/.opentrapp` directly is T4 and stays out of scope.

T1 and T2 are the principal threats the architecture is engineered against. T3 is a secondary concern addressed structurally (only one egress path, all of it logged). T4 is explicitly out of scope; the architecture does not promise mitigation when the host is compromised, and the documentation says so. T5 is addressed by user-experience measures rather than mechanical isolation. T6 is partially in scope. T7 is addressed structurally (the danger-gate) with an honest, T4-inherited residual.

The canonical threat-model document ([`docs/threat-model.md`](threat-model.md)) expands this summary into a STRIDE-style attacker-capability matrix with explicit residual-risk annotations and pointers to the empirical evidence supporting each mitigation.
...
- **Tier 2 (infrastructure).** The container perimeter. Enforces boundaries mechanically; does not make security decisions. Implemented by OpenTrApp's compose orchestration plus the five `vault-*` containers (vault-agent, vault-proxy, vault-egress, vault-skills, vault-social).
- **Tier 3 (contained).** The agent process, the Telegram gateway, loaded skills, and any fetched network content. Performs the work the user requests, within the boundaries Tier 2 enforces.
...
The five containers and their internal-network connectivity are summarised below; full detail is in [`docs/trifecta.md`](trifecta.md), [ADR-0009](adr/0009-five-container-perimeter.md), and the live `compose.yml`.

- **vault-agent** runs the agent runtime, the Telegram gateway, and any loaded skills. Read-only root filesystem, all Linux capabilities dropped, custom seccomp profile, no host filesystem mounts, no host network access. The `curl`/`wget` symlinks are removed so the agent has no convenient HTTP client (the busybox wget applet remains present). A single workspace directory is the only writable surface.
- **vault-skills** scans skills downloaded from ClawHub against an 87-pattern catalogue (16 of them prompt-injection patterns, all MITRE-mapped across 13 categories), runs a zero-trust line-classifier over each file, and rebuilds the skill from parsed semantic intent (CDR; §6). Output is delivered to vault-agent through a write-only shared volume; vault-skills has no path to vault-agent.
- **vault-social** vets untrusted feeds for prompt-injection patterns before content reaches vault-agent. The original Moltbook target was retired (the platform's acquisition and API instability) and a read-only AT Protocol (Bluesky) adapter shipped against the public AppView (ADR-0017); the module is **opt-in / off by default** and its full build-out as a generalized feed shield is **deferred** (ADR-0024 Thread C). The container is defined in `compose.yml` but does not start unless enabled.
...
Each of the three workload containers (`vault-agent`, `vault-skills`, `vault-social`) runs on its own Docker `internal: true` network. `vault-proxy` bridges those internal networks and reaches outbound only via `egress-net` to `vault-egress`. `vault-egress` is the only container on `external-net`. `vault-agent` cannot reach `vault-skills` or `vault-social` through any routed path; the only means by which a certified skill reaches `vault-agent` is the write-only shared volume `skills-deliveries`. The L7/L3 split is the load-bearing security property: no single container holds both API credentials *and* elevated network capabilities. This is the privilege-separation idea (one of the two the perimeter is built on) realised at the container layer: one contained agent, two guards.

### 3.3 Lifecycle ownership

The headless `opentrapp-daemon` owns the perimeter's lifetime. `opentrapp-daemon vault up` brings the perimeter up and supervises it; graceful termination (SIGTERM, SIGINT) tears it down synchronously with a 30-second ceiling; following SIGKILL, orphan containers are detected and stopped on the next launch via a PID-file mechanism (`RunGuard`). A *paused* state allows the perimeter to be suspended without stopping the daemon; it survives a daemon restart via the `~/.opentrapp/paused` marker file, and idle auto-pause drives it automatically ([ADR-0018](adr/0018-idle-auto-pause-host-waker.md)). Seven distinct termination paths were validated as cleanly tearing down the perimeter in the dogfood walkthrough (§8).

This is non-obvious in a way that affects security: a configuration-only hardening guide cannot make the same guarantee. The agent runtime configured in a hardening blog post is started at the user's discretion and stopped (or not) at the user's discretion; an enthusiastic user who clicks Quit may leave the runtime active in the background indefinitely, and a system that crashes leaves no record. The application-owns-perimeter discipline turns "is the agent currently exposed?" from a question of user vigilance into a structural property of the application's process lifetime.

The daemon + on-demand browser-viewer split has since **shipped** (de-Tauri, [ADR-0019](adr/0019-headless-daemon-gui-viewer-split.md) / [ADR-0022](adr/0022-daemon-control-surface.md)); the CLI-first command surface, registry-native distribution, and the optional MCP adapter described in [ADR-0020](adr/0020-product-identity-and-distribution.md) remain **forthcoming**.
...
The runtime, supply-chain, and feed-content categories below correspond to the canonical T1 (prompt-injection author), T2 (malicious skill author), and again T1 (feed content as a delivery vector for prompt injection). Hostile feed content is not a separate top-level threat in the canonical model; it is one way T1 content reaches the agent, and the `vault-social` shield is the optional second instance of the content-vetting pattern that addresses it.
...
Skills enter the agent's workspace through `vault-skills`. Six layers gate the path:
...
5. **Network isolation**: vault-skills has no routed path to vault-agent; certified skills are delivered through a write-only shared volume.
...
### 4.3 Hostile feed content (T1 delivery vector, social)

Feed content is one way an attacker delivers prompt injection (canonical T1). The optional `vault-social` container is a second instance of the content-vetting pattern, applied to that vector; it is opt-in / off by default (§3.2), with a read-only AT Protocol adapter shipped (ADR-0017). The layers:

1. **Feed scanner** (vault-social): a prompt-injection pattern set applied to untrusted feed content before it reaches the agent.
...
4. **Tool policy**: the agent has no tools that fetch from a social feed absent explicit user enablement.
5. **Coordinator approval**: the user retains visibility on every privileged action.

Layers 3 to 5 are functional regardless of `vault-social`'s status. Layers 1 and 2 are opt-in / off by default (a read-only AT Protocol adapter shipped; full build-out deferred).
...
**(a) The shell is a system state, not a setting.** It can be changed at runtime by a privileged actor (the user, or a trusted CLI coordinator). The agent itself cannot promote its own shell level; promotion is always initiated from Tier 1. Demotion ("shell up") is always permitted; promotion ("shell down") requires explicit confirmation. This is the danger-gate the canonical model relies on for T7: a boundary-weakening operation requires out-of-band human confirmation whether a human or an agent requests it.
...
The full mechanism is documented in [ADR-0002](adr/0002-adaptive-shell-levels.md). The implementation lives in [`workloads/agent/scripts/tool-control.sh`](../workloads/agent/scripts/tool-control.sh).
...
Content Disarm and Reconstruction inverts the question. Instead of asking whether the artefact is safe, the pipeline asks whether its *intent* can be re-expressed in a known-safe form. The original artefact is treated as a parse target, not as the deliverable. The pipeline runs in `vault-skills` as follows:

```
       download                   parse intent             rebuild from intent
ClawHub ────────► quarantine ────► structural model ────► clean skill artefact
                  (vault-skills)    (parser, in-container) (generator, in-container)
                       │                                            │
                       ▼                                            ▼
...
1. **The original artefact is never delivered.** Whatever side effects the original `SKILL.md` was supposed to produce (intentionally or by accident) are replaced by the side effects of the rebuilt version. An attacker who hides a payload in formatting tricks, in trailing whitespace, in `<!-- HTML comments -->`, in zero-width Unicode characters, or in layered base-64 encoding finds that none of those survive the parse-and-rebuild step.
...
The full mechanism is documented in [ADR-0003](adr/0003-content-disarm-reconstruction.md). The pipeline is implemented in [`workloads/skills/tools/skill-cdr.sh`](../workloads/skills/tools/skill-cdr.sh) and the `tools/lib/cdr-*` scripts.
...
The pre-cutover desktop delivery was implemented in Tauri 2 with a React 18 / TypeScript frontend and a Rust backend (the Tauri-over-Electron rationale — binary size, memory, attack surface — is in [ADR-0008](adr/0008-tauri-over-electron.md)). Since the de-Tauri cutover the product is the headless `opentrapp-daemon`: the Tauri-free Rust core (`crates/core`) houses the manifest orchestration logic and the perimeter lifecycle controller, the daemon (`crates/daemon`) owns the perimeter, and the same React frontend is served on demand by the loopback `viewer-server` (`crates/viewer-server`) instead of an in-process webview. The L7 chokepoint is the Go `goproxy` ([ADR-0026](adr/0026-vault-proxy-replacement-evaluation.md)), not the former Python mitmproxy addon.

The five containers (vault-agent, vault-skills, vault-social, vault-proxy, vault-egress) are produced from one workload or infra directory each, in a single monorepo since [ADR-0013](adr/0013-monorepo-consolidation.md): `workloads/agent/` produces vault-agent, `workloads/skills/` produces vault-skills, `workloads/social/` produces vault-social, `infra/proxy/` produces vault-proxy, and `infra/egress/` produces vault-egress. Each is defined by a `component.yml` manifest that conforms to a JSON Schema (`schemas/component.schema.json`). The manifest declares the component's identity, status states, runnable commands, editable configuration files, health probes, and workflows. The Tauri backend reads these manifests at startup; the React frontend renders dashboards generically from them. This generic-backend constraint (the application reads manifests and executes what they declare; it does not contain component-specific logic) is the central composability property that lets the five-container perimeter be reasoned about without hard-coding container knowledge into the orchestrator.

Schema alignment across three implementations (the JSON Schema, the Rust serde structs, the TypeScript types) is verified by [`tests/orchestrator-check.sh`](../tests/orchestrator-check.sh) on every commit. The 115-check suite covers manifest parsing, cross-reference validation (commands referenced from workflows, states referenced from `available_when`, orchestrator-workflow steps referencing component commands), and frontend-backend command parity (every Rust command handler has a matching TypeScript invoke wrapper).

The perimeter lifecycle is owned by the daemon (in the pre-cutover delivery, by the Tauri backend). It registers handlers for process exit, `SIGTERM`, and `SIGINT`, each of which tears the perimeter down synchronously with a 30-second ceiling. Following `SIGKILL`, the next launch reads `~/.opentrapp/runguard.pid`, observes that the previous PID is no longer alive, and reaps any orphan containers before bringing the perimeter back up. A user-initiated *paused* state is persisted as a marker file (`~/.opentrapp/paused`); the perimeter is not auto-started while the marker is present. The status aggregator (a Tokio interval task) re-evaluates the assistant's status (`ok` / `starting` / `recovering` / `error_perimeter` / `error_key` / `paused_by_user` / `not_setup`) every 60 seconds and emits a Tauri event on transition; the frontend Home view's hero state machine subscribes to this event.
...
The perimeter's correctness is verified at three scales: a per-startup container-hardening check, a per-commit manifest-orchestration suite, and an end-to-end browser harness that exercises the GUI's user-facing surfaces.

**Container hardening (24-point verification).** [`workloads/agent/scripts/verify.sh`](../workloads/agent/scripts/verify.sh) runs at container startup and on demand. The 24 checks fall into three groups: 14 universal-hardening checks (proxy DNS, proxy TCP, read-only root, capabilities dropped, no host mounts, no Windows interop, API-key absence from `vault-agent`'s environment, Docker socket not mounted, sudo unavailable, non-root user, seccomp loaded, `noexec` on `/tmp`, `no-new-privileges` set, PID limit), 4 shell-specific checks (profile matches active shell, exec security matches, host and elevated controls correct, safe-binary list matches profile), and 6 per-tool security checks (permanently-denied tools denied, `rm` not in safebins, no interpreters in safebins, proxy allowlist clean, risk score in range, configuration-integrity hash matches startup snapshot). All 24 checks pass in the current configuration.

**Manifest orchestration (115-check suite).** [`tests/orchestrator-check.sh`](../tests/orchestrator-check.sh) runs in CI on every push and pull request. The checks cover repository structure, JSON Schema validity, manifest parsing across all workloads, build artefacts, frontend-backend command parity, manifest enum alignment with Rust serde expectations, prerequisites cross-references, and workflow step to command references. The suite reports zero warnings in the current configuration.

**End-to-end browser harness.** Playwright tests in [`app/e2e/`](../app/e2e/) exercise the wizard flow, the user-mode pages, and the banned-term enforcement (28 developer-jargon terms verified absent from user-visible text). The full suite runs under one minute and is part of the standard pre-commit gate.

**Frontend unit tests.** Vitest tests in [`app/src/`](../app/src/) cover hooks, settings persistence, error classification, and component rendering. TypeScript strict mode is verified separately.

**Backend unit tests.** Rust tests in [`app/src-tauri/`](../app/src-tauri/) cover the manifest parser, the lifecycle module, the status aggregator (including the seven-state transition matrix), and the secret-redaction utility. All tests run without containers.

**Live end-to-end verification.** A separate Telethon-based harness in [`tests/e2e-telegram/`](../tests/e2e-telegram/) drives the agent from a real Telegram account against a running perimeter and observes responses. The harness produced eight Karen-persona scenarios in an early verification run, with combined Anthropic API spend under $0.04. No security regressions were observed; the test produced new banned-term findings (e.g. raw OpenClaw tool names appearing in bot replies under specific prompt shapes) which were added to the regression suite.

The Pass 8 pre-ship audit ([`docs/specs/2026-05-02-pass-8-preship-walk.md`](specs/2026-05-02-pass-8-preship-walk.md)) walked every reachable user-facing surface against a 13-principle UX rubric and against a "deserve-to-exist" scope test. Eight of nine non-wizard user surfaces scored at or above 8.5 / 10 on the rubric; the ninth (Telegram first-chat at 8.4) sits in a separate repository's system prompt and is tracked there. The audit issued a SHIP recommendation.
...
**vault-social is opt-in / off by default.** The social workload does not start unless enabled; a read-only AT Protocol adapter shipped (ADR-0017) but its full build-out is deferred (ADR-0024 Thread C). While it is off, the feed-content delivery vector for prompt injection (canonical T1) is mitigated only by the structural layers (network isolation, DM pairing policy, tool policy, coordinator approval); the active feed-scanner layer is not running. Enabling the workload activates the feed-scanner layer.
...
This paper has described a five-container perimeter for running the OpenClaw autonomous AI agent on a personal computer, with defense-in-depth across three threat categories, a stateful adaptive-shell mechanism that allows agent privilege to track task context, a Content Disarm and Reconstruction pipeline applied to skills downloaded from a third-party registry, and a load-bearing L7/L3 policy split between vault-proxy (application-layer egress) and vault-egress (network-layer egress + pinned DoT resolver) such that no single container holds both API credentials and elevated network capabilities (ADR-0009, ADR-0010). The architectural choice that most distinguishes this work from prior hardening guidance is the *composition*: not the individual primitives (capability dropping, seccomp, allowlist proxies) but their integration into a coherent perimeter that an end user can install and reason about.
...
**Ecosystem.** `vault-social` ships opt-in / off by default, with a read-only AT Protocol adapter (ADR-0017) and a deferred full build-out (ADR-0024 Thread C). As the agent-social category evolves, the architectural slot is in place and enabling the workload activates the perimeter layer.
...
- **agent runtime.** [getopenclaw.ai](https://www.getopenclaw.ai). Source repository: openclaw/openclaw.
...
- **ClawHavoc supply-chain study (2026-Q1).** 341 of 2,857 published ClawHub skills classified as malicious; full analysis at [`workloads/skills/docs/research/`](../workloads/skills/docs/research/).
...
- **Tauri 2.** [tauri.app](https://tauri.app). Pre-cutover GUI framework; removed in the de-Tauri cutover ([ADR-0022](adr/0022-daemon-control-surface.md)).
- **goproxy.** [github.com/elazarl/goproxy](https://github.com/elazarl/goproxy). The basis for `vault-proxy` ([ADR-0026](adr/0026-vault-proxy-replacement-evaluation.md)); replaced the earlier Python mitmproxy.
