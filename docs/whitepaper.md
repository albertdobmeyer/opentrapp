...
**T4: Compromised host.** An attacker who already has partial access to the user's machine — a malicious binary the user installed before this perimeter, an OS-level rootkit, or another foothold. Capability depends on the attack vector but is generally significant. Mitigation goal: explicitly out of scope. The perimeter assumes the host is honest; if the host is dishonest, no container-level isolation can recover security. Users requiring stronger isolation are directed to run the perimeter on a disposable virtual machine (see [`components/openclaw-vault/README.md`](../components/openclaw-vault/README.md) § Isolation tiers).
...
The full mechanism is documented in [ADR-0002](adr/0002-adaptive-shell-levels.md). The implementation lives in [`components/openclaw-vault/scripts/tool-control.sh`](../components/openclaw-vault/scripts/tool-control.sh).
...
The full mechanism is documented in [ADR-0003](adr/0003-content-disarm-reconstruction.md). The pipeline is implemented in [`components/clawhub-forge/tools/skill-cdr.sh`](../components/clawhub-forge/tools/skill-cdr.sh) and the `tools/lib/cdr-*` scripts.
...
- **ClawHavoc supply-chain study (2026-Q1).** 341 of 2,857 published ClawHub skills classified as malicious; full analysis at [`components/openclaw-vault/docs/research/`](../components/openclaw-vault/docs/research/) (companion repository).
