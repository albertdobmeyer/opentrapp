...
OpenTrApp is a **registry-installable CLI/daemon orchestrator + signed container images** that runs an autonomous CLI agent inside a **five-container security perimeter** on the user's own machine — **operable by humans *and* by the user's host agent** (e.g. Claude Code) — with a web GUI and an optional MCP adapter as **thin, on-demand projections of the same manifest-driven daemon**. The daemon owns the perimeter; the projections come and go. The *external* operator orchestrates the *contained* agent, and **boundary-weakening operations stay human-gated regardless of who asks** (danger-gated control plane; [ADR-0021](docs/adr/0021-danger-gated-agentic-control-plane.md) — the gate + the out-of-band GUI two-tap approval surface are **built** on the control channel + viewer-server). It is **not** a desktop app and **not** an MCP server for the *contained* agent.

**Current state (honest — the identity above is the TARGET, partly built):** The **de-Tauri cutover has SHIPPED on `main`** ([#184](https://github.com/albertdobmeyer/opentrapp/pull/184), 2026-06-24): the Tauri/GTK3 GUI crate is **deleted**, and the product is now the headless `opentrapp-daemon` + an on-demand **loopback browser viewer** (`viewer-server`), **GTK-free** (the 19 GTK3 advisories cleared; `Cargo.lock` has 0 tauri/wry/webkit entries; the `deny.toml` ignore list is empty). So **de-Tauri IS done and the browser-viewer is real** — document those as existing. What is NOT yet built: signed cross-platform **installers** for the new architecture (cargo-dist, [ADR-0023](docs/adr/0023-distribution-and-packaging.md) — pending, so the new build is from-source today and the last tagged release v0.8.0 is still the previous Tauri app), and the **registry / MCP / CLI-first command surface** ([ADR-0020](docs/adr/0020-product-identity-and-distribution.md) / [0022](docs/adr/0022-daemon-control-surface.md) — forthcoming). **Do not document the registry / MCP / CLI-first command surfaces as existing until they land.** The [ADR-0021](docs/adr/0021-danger-gated-agentic-control-plane.md) **danger-gate IS built** (the control-inbox holds a boundary-weakening request; only the out-of-band GUI two-tap applies it; the manifest `boundary_impact` axis is fail-closed + CI-validated), and every future surface inherits it unchanged. The `vault-proxy` replacement is decided but not yet built (goproxy, [ADR-0026](docs/adr/0026-vault-proxy-replacement-evaluation.md)).
