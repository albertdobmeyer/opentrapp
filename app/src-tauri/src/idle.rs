...
async fn resume_from_dormant(data_dir: &Path) {
    if !crate::lifecycle::is_dormant_persisted() {
...
/// Spawn the waker for a freshly-dormant perimeter. No-op (with a log) when no
/// bot token is configured — without a token there is nothing to poll. Replaces
/// and cancels any previously-stored waker.
pub fn spawn_waker(app: AppHandle, data_dir: PathBuf) {
    ...
    let dir_task = data_dir.clone();
    let handle = tokio::spawn(async move {
        waker_loop(dir_task, token, cancel_task).await;
    });
    ...
    if let Some(state) = app.try_state::<AppState>() {
        if let Ok(mut guard) = state.waker.lock() {
            ...
            *guard = Some(IdleWaker { cancel, handle });
        }
    }
}
