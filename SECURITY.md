...
Vulnerabilities in upstream dependencies (Tauri, mitmproxy, Rust crates, npm packages) and in the third-party platforms this software interfaces with (Anthropic API, Telegram, OpenClaw, ClawHub, Moltbook) are out of scope; please report those to their respective maintainers. In particular, vulnerabilities in the **contained agent runtime itself** (the open-source CLI agent inside `vault-agent`, e.g. OpenClaw, and its dependency tree) are out of scope by design: that agent is the Tier-3 untrusted-by-design payload the perimeter *contains* rather than trusts, so its own flaws are the threat model, not a defect in OpenTrApp. Advisories we knowingly accept (chiefly the unmaintained Tauri GTK3 webview crates), the **trust-tier triage** that decides which alerts are release-gating versus contained residuals, and how to read the project's OpenSSF Scorecard are documented in [`docs/known-advisories.md`](docs/known-advisories.md).

The full attacker-capability matrix (T1–T7, where T7 is the prompt-injected host operator introduced by the agent-operable control plane), the perimeter layers that mitigate each, and the residual risks that remain after mitigation are documented in [`docs/threat-model.md`](docs/threat-model.md). The "In scope" and "Out of scope" sections below are aligned with that document.
