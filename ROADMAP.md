...
*Gate:* all green via the product path. (Section 1.) T0 ✅ (2026-06-26); T1 mechanism ✅ + T2 security-correct resume ✅ (#195/#197), with the live wake-on-real-message the named maintainer tail (#35, #40); #76 (first-run image staging) ✅ (#200). **The Rung-1 substance is met; what remains is the owner-gated release cut (Section 5).**
...
|---|---|---|
| T0 boundary self-test, cold == resumed | ✅ | **Verified via the product CLI 2026-06-26 on the live goproxy perimeter** (not dev scaffolding): `opentrapp-daemon vault up` → `vault verify` `pass=7 fail=0` cold, then `vault pause`→`resume`→`vault verify` `pass=7 fail=0` with **B5 CA-fingerprint UNCHANGED**. End-user-faithful (the product daemon's native orchestrator, not `podman-compose`). Scope: DevVerifier/from-source mode + a placeholder key (B1–B6 need no valid key — P1-1); the BundleVerifier digest-staging path is the post-release T0, exercised once the owner tags v0.9.0 and the signed assets exist (the lane itself is now built — #199/#200). |
| WS0-0a: idle auto-pause actually fires (T1) | 🔶 | **Always-on, token-gated** — the `IDLE_AUTO_PAUSE_ENABLED` const was removed (2026-06-09); the supervisor auto-pauses whenever a Telegram token is present (`should_auto_pause`; fail-safe). **Mechanism verified on the box 2026-06-27** via the `OPENTRAPP_IDLE_TIMEOUT_MS` knob: idle past the threshold → `[supervisor] pausing to dormant` → DORMANT marker + all containers stopped; control-resume → auto-self-test PASS → awake. **Maintainer tail:** the full RAM-collapse magnitude needs a real agent loaded (a placeholder OpenClaw never loads its ~356 MB working set), and the live wake-on-real-message needs a real token. |
